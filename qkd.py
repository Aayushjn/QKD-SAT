...
def breaking_probabilities(
    gathering_probability: np.ndarray[np.float64], decoding_probability: np.ndarray[np.float64]
...
def objective_value(
    ...
    num_parts = len(paths)
    gathering_probabilities = np.array(
        [gathering_probability(i, paths, collaboration_matrix) for i in range(num_nodes)]
    )
    decoding_probabiliities = np.array(
        [decoding_probabiliity(i, num_parts, curiosity_matrix) for i in range(num_nodes)]
    )
...
def path_breaking_probability(
    ...
    return max(
        (curiosity_matrix[node] ** num_shares) * gathering_probability(node, paths, np.array(collaboration_matrix))
        for node in path[1:-1]
    )


def optimality(
    paths: list[NetworkPath], curiosity_matrix: tuple[np.float64], collaboration_matrix: tuple[tuple[np.float64, ...]]
) -> tuple[int, tuple[int, ...]]:
    optimal_parts = -1
    max_q_value = 1
