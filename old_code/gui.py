...
class Simulation(backend.NetworkGraph):
    ...
    def update_gathering_probability(self, gathering_probability):
        self.update_1d_metric(
            WIDTH * 3 / 4, HEIGHT * 4 / 8, WIDTH / 4, HEIGHT / 8, gathering_probability, "GATHERING PROBABILITY"
        )

    def update_decoding_probability(self, decoding_probability):
        self.update_1d_metric(
            WIDTH * 3 / 4, HEIGHT * 5 / 8, WIDTH / 4, HEIGHT / 8, decoding_probability, "DECODING PROBABILITY"
        )

    def update_breaking_probability(self, breaking_probability):
        self.update_1d_metric(
            WIDTH * 3 / 4, HEIGHT * 6 / 8, WIDTH / 4, HEIGHT / 8, breaking_probability, "BREAKING PROBABILITY"
        )

    def create_node(self, n, width, height):
        return (
            Simulation.create_source(width, height)
            if n == self.start_node
            else (
                Simulation.create_destination(width, height)
                if n == self.end_node
    ...
    def update_objective_value(self, value=0.0):
        ...
        surface, rectangle, width, height = Simulation.construct_surface(
            WIDTH * 3 / 4, HEIGHT * 7 / 8, WIDTH / 4, HEIGHT / 8
        )
    ...
    def draw_buttons(self):
        self.create_button(
            Simulation.scaled_rect(WIDTH * 2 / 8, HEIGHT * 7 / 8, WIDTH / 8, HEIGHT / 8, 0.7),
            "NEXT",
            RED,
            DARK_RED,
            onclick_handler=self.simulation_next,
        )
        self.create_button(
            Simulation.scaled_rect(WIDTH * 3 / 8, HEIGHT * 7 / 8, WIDTH / 8, HEIGHT / 8, 0.7),
            "PLAY",
            RED,
            DARK_RED,
            onclick_handler=self.simulation_play,
        )
        self.create_button(
            Simulation.scaled_rect(WIDTH * 4 / 8, HEIGHT * 7 / 8, WIDTH / 8, HEIGHT / 8, 0.7),
            "START",
            RED,
            DARK_RED,
            onclick_handler=self.start_simulation,
        )
        self.create_button(
            Simulation.scaled_rect(WIDTH * 5 / 8, HEIGHT * 7 / 8, WIDTH / 8, HEIGHT / 8, 0.7),
            "RESET",
            RED,
            DARK_RED,
            onclick_handler=self.reset_simulation,
        )
    ...
    def start_simulation(self, obj_fn=backend.ProbabilisticModel.ObjectFunction.NO_NODE_BREAK_SECRET):
        ...
        self.obj_fn = obj_fn
        # TODO:
        self.simulation = backend.simulator(
    ...
    def update_measures(
        ...
        optimized,
        chosen_paths,
        gathering_probability,
        decoding_probability,
