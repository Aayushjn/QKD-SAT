...
class Point:
    ...
    @staticmethod
    def euclid_distance(p1: "Point", p2: "Point"):
...
class NetworkGraph(nx.Graph):
    ...
    def __calculate_distances__(self):
        ...
        distances = [
            (Point.distance(self.nodes[i]["point"], self.nodes[j]["point"]), i, j)
            for i in range(self.num_nodes)
            for j in range(i)
        ]
    ...
    def calculate_layout_points(self):
        ...
        for i in range(self.num_nodes):
            self.nodes[i]["layout_point"] = Point.normalize(
                *spring_positions[i], x_min=x_min, x_max=x_max, y_min=y_min, y_max=y_max
            )
