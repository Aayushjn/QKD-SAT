...
def random_curiosity_matrix(num_nodes: int, low: float = 0.0, high: float = 1.0) -> npt.NDArray[np.float64]:
...
def random_collaboration_matrix(num_nodes: int, low: float = 0.0, high: float = 1.0) -> npt.NDArray[np.float64]:
    rng = np.random.default_rng(seed=int.from_bytes(os.urandom(4), sys.byteorder))
